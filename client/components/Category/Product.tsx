...
const Product: FC<CategoryProduct & { index: number }> = ({ index, ...props }) => {
    ...
    return (
        <div className={styles.container} style={{ flexDirection: getStyle() }}>
            <picture>
                <source media="(max-width: 590px)" srcSet={props["category-thumbnails"].mobile} />
                <source media="(max-width: 900px)" srcSet={props["category-thumbnails"].tablet} />
                <img src={props["category-thumbnails"].desktop} alt="category-product" />
            </picture>
            <section>
